...
func Test_intervalFromCommand(t *testing.T) {
	tests := []struct {
		...
		{cmd: "/home/weave/scope --mode=probe --probe-only --probe.kubernetes.role=host --probe.publish.interval=4500ms --probe.spy.interval=1s --probe.docker.bridge=docker0 --probe.docker=true --probe.ebpf.connections=false --probe.conntrack=false https://redacted@cloud.weave.works.", want: "4500ms", name: "lower-spy-interval"},
